...
# TRAINING
DATASET_RGB = dir_path+"/data/rgb/*.tif"
DATASET_GTI = dir_path+"/data/gti/*.tif"
DATASET_SEG = dir_path+"/data/seg/*.tif"

DEBUG_DIR = dir_path+"/debug/"

# INFERENCE
INF_RGB = dir_path + "/test_data/rgb/*.tif"
INF_SEG = dir_path + "/test_data/seg/*.tif"
INF_OUT = dir_path + "/test_data/reg_output/"

MODEL_ENCODER = dir_path+"/saved_models_gan/E140000_e1"
MODEL_GENERATOR = dir_path+"/saved_models_gan/E140000_net"
...
def regularize_segmentations(img_folder, seg_folder, out_folder, in_mode="semantic", out_mode="instance", samples=None):
    ...
    for num, (satellite_image_file, building_segmentation_file) in enumerate(zip(img_files, seg_files)):
        ...
        print("building_segmentation_file",building_segmentation_file)
        ...
        if out_mode == "instance":
            ...
            R_stretched = exposure.rescale_intensity(R, in_range='image', out_range=np.uint16)
            io.imsave(output_file, R_stretched)
            # Display the stretched image using matplotlib
...
def run_regularize():
    regularize_segmentations(img_folder=INF_RGB, seg_folder=INF_SEG, out_folder=INF_OUT, in_mode="semantic", out_mode="instance", samples=None)
    print("DONE REGULARIZE FINALS")
    return True